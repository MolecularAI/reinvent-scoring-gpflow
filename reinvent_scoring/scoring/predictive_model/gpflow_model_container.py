...
class GPflowModelContainer(BaseModelContainer):
  def __init__(self, activity_model, model_type: str, specific_parameters: {}):
    """
    :type activity_model: GPflow type of model object
  ...
  def predict(self, molecules: List, parameters: Dict) -> np.array:
    """
    Takes as input RDKit molecules and uses a pickled GPflow model to predict activities.
  ...
  def predict_from_fingerprints(self, fps):
    if self._model_type == "regression":
        activity,_ = self._activity_model.predict_y_compiled(fps)
    else:
        activity,_ = self._activity_model.predict_y_compiled(fps)
