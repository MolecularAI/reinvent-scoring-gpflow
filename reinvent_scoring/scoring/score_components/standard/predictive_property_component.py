...
class PredictivePropertyComponent(BaseScoreComponent):
    ...
    def _load_container(self, parameters: ComponentParameters):
        container_type = parameters.specific_parameters.get(self.component_specific_parameters.CONTAINER_TYPE,
                                                            self.container_type.SCIKIT_CONTAINER)
        model_path = self.parameters.specific_parameters.get(self.component_specific_parameters.MODEL_PATH, "")
        if container_type == self.container_type.SCIKIT_CONTAINER:
            with open(model_path, "rb") as f:
                activity_model = pickle.load(f)
                f.close()
        elif container_type == self.container_type.GPFLOW_CONTAINER:
            activity_model = tf.saved_model.load(model_path)
        packaged_model = ModelContainer(activity_model, parameters.specific_parameters)
